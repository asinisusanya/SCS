...
import { Calendar, AlertCircle, Info, Clock, Bell, Star, Users } from "lucide-react"
...
export default function NoticesScreen() {
    ...
    const loadNoticesData = async () => {
      try {
        ...
        if (adminNotices) {
          ...
          const activeAdminNotices = parsedAdminNotices.filter((notice: any) => notice.isActive !== false)
          if (activeAdminNotices.length > 0) {
            ...
            const sortedAdminNotices = activeAdminNotices.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime())
  ...
  const getPriorityStyle = (priority: Notice["priority"]) => {
    switch (priority) {
      ...
      case "high":
        return "border-l-[#f59e0b] bg-gradient-to-r from-[#f59e0b]/20 to-[#f59e0b]/5 shadow-[#f59e0b]/20"
      case "medium":
        return "border-l-blue-500 bg-gradient-to-r from-blue-500/20 to-blue-500/5 shadow-blue-500/20"
      default:
  ...
  const criticalNotices = notices.filter((n) => n.priority === "critical").length
    ...
    return (
      <div className="h-screen bg-gradient-to-br from-[#1e3a8a] via-[#1e40af] to-[#3b82f6] text-white overflow-hidden flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-[#f59e0b] mb-2">Loading Notices</h2>
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-[#1e3a8a] via-[#1e40af] to-[#3b82f6] text-white overflow-hidden">
      {/* Enhanced Header */}
      <div className="bg-black/20 backdrop-blur-sm border-b border-white/20 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-[#f59e0b] mb-1 flex items-center gap-3">
              <Bell className="w-8 h-8" />
              Notice Board
            </h1>
            <p className="text-lg font-semibold text-white flex items-center gap-2">
              Latest Updates & Important Information
              {error && <span className="text-sm text-red-300 bg-red-500/20 px-2 py-1 rounded">Using cached data</span>}
            </p>
          </div>

          <div className="flex items-center gap-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-red-400">{criticalNotices}</div>
              <div className="text-sm text-white/80">Critical</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-[#f59e0b]">{newNotices}</div>
      ...
      <div className="p-4 h-[calc(100vh-140px)]">
        {notices.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">No Active Notices</h2>
              <p className="text-white/70">All notices are up to date</p>
            </div>
          </div>
        ) : (
          <div className={`grid gap-4 h-full ${notices.length === 1 ? "grid-cols-1" : notices.length === 2 ? "grid-cols-2" : notices.length === 3 ? "grid-cols-3" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"}`}>
            {displayedNotices.map((notice, index) => (
              <div
                ...
                `}
                style={{
                  animationDelay: `${index * 0.08 + Math.floor(index / 3) * 0.05}s`,
              ...
              >
                ...
                <div className="p-4 border-b border-white/10">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <div className="text-[#f59e0b]">{getIcon(notice.type)}</div>
                      <div>
                        <h3 className="text-base font-bold text-[#f59e0b] line-clamp-1">{notice.title}</h3>
                        {notice.category && <span className="text-xs text-white/60">{notice.category}</span>}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getTypeColor(notice.type)}`}>
                ...
                <div className="flex-1 p-4">
                  <p className="text-sm text-white leading-relaxed line-clamp-4 mb-3">{notice.message}</p>
        ...
        {notices.length > 6 && (
          <div className="mt-4 flex items-center justify-center gap-2">
            {Array.from({ length: Math.ceil(notices.length / 6) }).map((_, index) => (
              <div
                key={index}
                className={`w-2 h-2 rounded-full transition-all ${
                  Math.floor(currentNoticeIndex / 6) === index ? "bg-[#f59e0b]" : "bg-white/30"
                }`}
              />
            ))}
          </div>
